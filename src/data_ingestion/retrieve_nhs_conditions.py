...
# below is pulled from  https://github.com/nhsx/language-corpus-tools/tree/master
...
class NHSConditionsAPISpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        return [scrapy.Request(url="https://www.google.com")]
    ...
    def parse_item(self, response):
        ...
        if response.status == 200:
            ...
            if "https://www.nhs" in response.url:
                # then it's a weblink, meaning we need to use a different method to extract the text
                 
                soup = BeautifulSoup(response.text)
                date_modified = json.loads(soup.script.text)['dateModified']

                html = str(soup)

                for h in soup.find_all('li'):
                    if 'nhsuk-hub-key-links__list-item' in h.attrs.get['class']:
                        a = h.find('a')
                        print(a.get('href'))

                        html += "/n"
                        html += str(BeautifulSoup(requests.get(a.get('href')).text))
            else:
                try:
                    rjson = json.loads(response.text)
                    ...
                    date_modified = rjson["dateModified"]
                except KeyError:
                    html = ""
                # if a normal web link is provided - get the text html out of those pages.

            item = HTMLItem(
                html=html,
                source_url=response.url,
                retrieved_at=datetime.datetime.now(),
                modified_at=dateutil.parser.parse(date_modified),
            )
        else:
            item = HTMLItem(
